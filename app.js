...
//new section and then creat a filter function for pokemon. starter t find the starter data from the pokemon array
...
game.catchPokemon = function(pokemonObj) {
    //define the function pokemonObj this lets it attahc to the game
    game.party.push(pokemonObj);
};


game.catchPokemon(pokemon[37]); 
//calling pokemon number 38! Ninetales!!! 
...
game.catchPokemon = function(pokemonObj) {
    ...
    const pokeballCount = game.items[1]
    if (pokeballCount){
        pokeballCount.quantity--;
    }
};

game.catchPokemon(pokemon[73])

//why are you not working??? ahhh ):  oops i cant spell!  CHECK YOUR SPELLING TAIHLOR!!!!!!
...
for (let i = 0; i < game.gyms.length; i++) {
    if (game.gyms[i].difficulty < 6) {
        game.gyms[i].completed = true; // i can just run the same code?  and it works?
}
}
...
game.partyCount = function() {
    ...
    // display the number of pokemon in the party
};
